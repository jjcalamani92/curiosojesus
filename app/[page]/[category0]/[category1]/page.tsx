...
async function getCategories() {
  const res = await fetch(`${process.env.SITE_URL}/api/portfolio/categories1/siteId/${process.env.SITE_UID}`, {method : "GET", cache: 'no-store'});
  if (!res.ok) {
    console.log('error');
...
async function getCategory(siteId: string, slug: string) {
  const res = await fetch(`${process.env.SITE_URL}/api/portfolio/categories1/category/${siteId}/${slug}`, {method : "GET", cache: 'no-store'});
  if (!res.ok) {
    console.log('error');
  }
  return res.json();
